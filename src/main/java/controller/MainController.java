...
//    }
    ...
    public static String ProductFind(String refreshToken) {

        return "ProductFindResult%error%Invalid refreshToken";
    }
    ...
    public static String getProductAccess(int productId) {
        String transform = "";
        ProductDAO pro = new ProductDAO();
        Product product = pro.getProductById(productId);

        if (product != null) {
            transform = "Product%&productID$" + product.getProductID()+ "%&productName$"+ product.getProductName()+
                    "%&productStock$"+ product.getProductStock()+ "%&productPrice$"+ product.getProductPrice();
        }
        else {
            transform = "Product%&error%getProductAccess";
        }

        return transform;
    }

    public static String purchaseProduct(int productId) {
        String transform = "";
        ProductDAO pro = new ProductDAO();
        Product product = pro.getProductById(productId);
        boolean stockDecreased = pro.decreaseProduct(productId);

            // 3. 모든 작업이 성공했는지 확인
        if (stockDecreased) { // && orderCreated 등...
            transform = "Purchase%&productID$" + product.getProductID()+ "%&productName$"+ product.getProductName()+
                    "%&productStock$"+ product.getProductStock()+ "%&productPrice$"+ product.getProductPrice();
        }
        else {
            transform = "Purchase%&error%StockDecreased";
